...
subtitle: "Interactive browser for T2D genetic associations with 23 insulin sensitivity indices"
format:
  dashboard:
    scrolling: true
    theme: cosmo
    nav-buttons:
      - icon: github
        href: https://github.com/USERNAMEsufyansuleman/isi_t2d_nat_24
execute:
  echo: false
  warning: false
  message: false
...
<small>If you use data from this resource, please cite:<br>
Suleman S, Ängquist L, Linneberg A, Hansen T, Grarup N.<br>
*Exploring the genetic intersection between obesity-associated genetic variants and insulin sensitivity indices.*<br>
**Sci Rep.** 2025;15(1):15761.<br>
[doi:10.1038/s41598-025-98507-w](https://doi.org/10.1038/s41598-025-98507-w) · PMID: 40328835</small>
...
```{ojs}
{
  ...
  return html`
    <table style="font-size:0.8rem;width:100%;border-collapse:collapse">
      <tr><td style="color:#555">Variants shown</td><td align="right"><b>${nVar}</b></td></tr>
      <tr><td style="color:#555">Unique genes</td><td align="right"><b>${nGene}</b></td></tr>
      <tr><td style="color:#555">Sig. assoc. (p&lt;0.05)</td><td align="right"><b>${nSig}</b></td></tr>
    </table>`;
}
```
...
// Group → colour band for background stripe
GROUP_COLOR = ({
  "Fasting":       "#f0f4ff",
  "OGTT,0-120":    "#fff7f0",
  "OGTT,0-30-120": "#f0fff4"
})
...
// Variants in their original (clustered) order, deduplicated
uniqueVariants = [...new Set(filtered.map(d => d.gene_snp_ra))]

// Visible x-axis indices
...
cellW    = 28
plotW    = Math.max(600, visibleIndices.length * cellW + 280)
...
Plot.plot({
  ...
  color: {
    ...
    legend:  true,
    label:   "Effect size (β)",
    reverse: true    // red = positive (higher IS)
  },
  ...
  marks: [
    ...
    // Black border for nominally significant (p < 0.05)
    sigHighlight
      ...
      : Plot.dot([], {}),   // no-op when toggle is off

    // White star for genome-wide / Bonferroni significant
...
## Gene Explorer {.tabset}

### By Gene – top associations

```{ojs}
//| output: false
// For each gene, compute best p-value across all indices and analyses
d3 = require("d3-array@3")

// Roll up to gene × is_index level: pick min pvalue, mean beta
